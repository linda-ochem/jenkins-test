...
function SingleUserTransactionPanel({
  userTransactionPanelOpen,
  setUserTransactionPanelOpen,
  selectedItems,
}) {
  ...
  useEffect(() => {
    const clickHandler = ({ target }) => {
      if (
        ...
        return;
      setUserTransactionPanelOpen(false);
    };
  ...
  useEffect(() => {
    const keyHandler = ({ keyCode }) => {
      if (!userTransactionPanelOpen || keyCode !== 27) return;
      setUserTransactionPanelOpen(false);
    };
  ...
  return (
    <div
      ...
      className={`absolute inset-0 sm:left-auto z-20 shadow-xl transition-transform duration-200 ease-in-out ${
        userTransactionPanelOpen ? "translate-x-" : "translate-x-full"
      }`}
    >
      <div className="sticky top-16 bg-slate-50 overflow-x-hidden overflow-y-auto no-scrollbar shrink-0 border-l border-slate-200 w-full sm:w-[390px] h-[calc(100vh-64px)]">
        <button
          ref={closeBtn}
          onClick={() => setUserTransactionPanelOpen(false)}
...
export default SingleUserTransactionPanel;
