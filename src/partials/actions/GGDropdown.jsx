...
function GGDropdown({ options, setSelected, selected }) {
  ...
  return (
    <div className="relative inline-flex w-full">
      <button
        ref={trigger}
        className="btn w-full justify-between bg-slate-200 border-0 hover:border-slate-300 text-blue-900 "
      ...
      >
        <span className="flex items-center">
          <span>{options[selected]?.transactionStatus}</span>
        </span>
        <svg
          ...
          {options.map((option) => {
            return (
              <button
                ...
                onClick={() => {
                  setSelected(option.transactionStatus);
                  setDropdownOpen(false);
                }}
              >
                <span>{option.transactionStatus}</span>
                <svg
