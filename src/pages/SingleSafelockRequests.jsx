...
function SingleSafeLockRequests(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await sendGetRequest2(
          `/admin/break-safe-requests?userId=${
            props?.userId
          }&page=${pageNumber}&limit=10${queryString ? `&${queryString}` : ""}`
        );
    ...
    fetchData();
  }, [filterParams, props?.userId, pageNumber]);
...
export default connect(mapStateToProps)(SingleSafeLockRequests);
