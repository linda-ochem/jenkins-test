...
function ElementDeposits(props) {
  // console.log(props);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const response = await sendGetRequest2(
          `/admin/transactions?type=DEPOSIT_TO_WALLET&description=Deposit to Element Account&currency=USD&page=${pageNumber}&limit=50${
            queryString ? `&${queryString}` : ""
          }`
        );
        // console.log(response);
  ...
  const handleSubmit = async (selectedRecord) => {
    ...
    setLoading(true);
    try {
      const res = await depositUpdate(status, trxRef, userId);
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <ModalBlank
        ...
        title="Update Element Deposit Transaction"
      >
        {/* Modal content */}
        <div className="px-5 py-4">
          <form>
...
export default connect(mapStateToProps)(ElementDeposits);
