...
import { approveDeclineWithdrawalRequests } from "../actions";
...
function WithdrawalRequests(props) {
  ...
  const [totalAmountGHS, setTotalAmountGHS] = useState(null);
  ...
  //makes the call to fetch all withdrawal sum GHS
  useEffect(() => {
    const fetchGHSData = async () => {
      try {
        setLoading(true);
        const response = await sendGetRequest2(
          `/admin/withdrawal-requests-sum?currency=GHS`
        );
        if (response.data) {
          setLoading(false);
          // console.log(response);
          // console.log(response.data?.data);
          setTotalAmountGHS(response.data?.data?.data);
        } else {
          setLoading(false);
  ...
  //makes the call to fetch all withdrawal sum NGN
  useEffect(() => {
    const fetchNGNData = async () => {
      try {
        setLoading(true);
        const response = await sendGetRequest2(
          `/admin/withdrawal-requests-sum?currency=NGN`
        );
        if (response.data) {
          setLoading(false);
          // console.log(response);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setLoading(true);
        const response = await sendGetRequest2(
          `/admin/withdrawal-requests?page=${pageNumber}&limit=10${
  ...
  const handleAction = (requestId = id, status = action) => {
    Modal.confirm({
      ...
      onOk: async () => {
        setLoading(true);
        try {
          const res = await approveDeclineWithdrawalRequests(
            requestId,
            status,
            reason
          );
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "amount",
      render: (text, record) => {
        return toWholeCurrency(record.amount);
      },
      width: 120,
    },
    {
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <input
                        ...
                        defaultValue={toWholeCurrency(selectedUser.amount)}
                        required
                        disabled
                      />
                    </div>
...
export default connect(mapStateToProps)(WithdrawalRequests);
